...
docker run --rm -d -e THREAD_MODE=multi -e LD_PRELOAD="/usr/lib64/libjemalloc.so.2" --name "Meetup_Jemalloc" --cpus 4 -m 2g meetup
docker run --rm -d -e THREAD_MODE=multi -e LD_PRELOAD="/usr/lib64/libmimalloc.so.2.1" --name "Meetup_Mimalloc" --cpus 4 -m 2g meetup
